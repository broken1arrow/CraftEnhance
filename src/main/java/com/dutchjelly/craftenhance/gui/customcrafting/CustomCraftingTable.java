...
public class CustomCraftingTable extends com.dutchjelly.craftenhance.gui.guis.GUIElement {
	...
	public CustomCraftingTable(GuiManager manager, GuiTemplate template, GUIElement previous, Player p) {
		...
		inventory = GuiUtil.CopyInventory(getTemplate().getStacksTemplate(), getTemplate().getInvTitle(), this);
	...
	public void handleEventRest(InventoryClickEvent e) {
		final GuiTemplate template = getTemplate();
		final List<Integer> fillSpace = template.getFillSpace();
		if (template.getResultSlot() != e.getSlot() && !fillSpace.contains(e.getSlot())) {
			Debug.info("player clicked outside recipe");
			e.setCancelled(true);
			return;
		}
		...
		if (template.getResultSlot() != e.getSlot()) {
			...
			Debug.info("player clicked inside recipe content");
		...
		//Result slot clicked.
		Debug.info("player clicked result slot");
		...
		if (e.getClick().equals(ClickType.SHIFT_LEFT) || e.getClick().equals(ClickType.SHIFT_RIGHT)) {
			...
			Map<Integer, Integer> destination = GuiUtil.findDestination(
					...
					getPlayer().getInventory().getContents().length * result.getMaxStackSize(),
					false, null
			);
			...
			if (findFittingRecipesAmount() == 0) {
				throw new IllegalStateException("recipe does not match, the inventory is empty or not match a recipe");
				// return;
			}

			ItemStack onCursor = getPlayer().getOpenInventory().getCursor();
			if (!GuiUtil.isNull(onCursor)) {
				Debug.info("result has to add to nonempty cursor: " + onCursor.getType());
				...
				Debug.info("setting empty cursor to result item");
		...
		Debug.info("Player is crafting " + canCraft + " recipes");
	...
	public void handleOutsideClick(InventoryClickEvent e) {
		...
		Debug.info("Player shift clicked item in own inventory");

		e.setCancelled(true);

		final List<Integer> fillSpace = getTemplate().getFillSpace();
		Map<Integer, Integer> destination = GuiUtil.findDestination(
				e.getCurrentItem(), getInventory(), -1, false,
				fillSpace.subList(0, 9)
		);
	...
	public void handleDragging(InventoryDragEvent e) {
		...
		Debug.info("player dropped " + dropped + " item");
		ItemStack newCursor = e.getOldCursor().clone();
		if (dropped == newCursor.getAmount())
			CraftEnhance.runTask(() -> e.getView().setCursor(null));
		else {
			newCursor.setAmount(newCursor.getAmount() - dropped);
			CraftEnhance.runTask(() -> e.getView().setCursor(newCursor));
		}
...
//        }
	...
	public ItemStack getResult() {
		return inventory.getItem(getTemplate().getResultSlot());
	}
	...
	public void setMatrix(ItemStack[] matrix) {
		for (int i = 0; i < matrix.length; i++) {
			Debug.info("setting item " + i + " to " + matrix[i]);
