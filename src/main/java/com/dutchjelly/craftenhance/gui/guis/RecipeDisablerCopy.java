...
public class RecipeDisablerCopy extends MenuHolder {
	...
	public RecipeDisablerCopy(List<Recipe> enabledRecipes, List<Recipe> disabledRecipes, boolean enableMode, String recipesSeachFor) {
...
//               getRecipes().remove(recipe);
	...
	public boolean run(com.dutchjelly.craftenhance.gui.templates.MenuButton value, Inventory menu, Player player, ClickType click) {
		...
		if (value.getButtonType() == ButtonType.SwitchDisablerMode){
			...
			new RecipeDisablerCopy(RecipeLoader.getInstance().getServerRecipes(),RecipeLoader.getInstance().getDisabledServerRecipes(),this.enableMode,"").menuOpen(player);
			return true;
		}
			...
			if (click == ClickType.RIGHT) {
				...
				self().getGuiManager().waitForChatInput(this, getViewer(), msg -> {
					if (GuiUtil.seachCategory(msg)) {
						new RecipeDisablerCopy(RecipeLoader.getInstance().getServerRecipes(), RecipeLoader.getInstance().getDisabledServerRecipes(), this.enableMode, msg).menuOpen(getViewer());
						return false;
					}
					return true;
				});
			}
			else new RecipeDisablerCopy(RecipeLoader.getInstance().getServerRecipes(),RecipeLoader.getInstance().getDisabledServerRecipes(), this.enableMode, "").menuOpen(player);
		}
		return false;
	}
