...
public class FileManager {
    ...
    public static FileManager init(final CraftEnhance main) {
        ...
        // load containerOwnerConfig
        fm.containerOwnerConfig = fm.getYamlConfig(fm.containerOwnerFile);
		fm.serverRecipeConfig = fm.getYamlConfig(fm.serverRecipeFile);
        return fm;
    }
    @SneakyThrows
    ...
    private File ensureCreated(final File file) {
        if (!file.exists()) {
            ...
            try {
                ...
                logger.warning("The file " + file.getName() + " couldn't be created!");
            }
        }
        return file;
    }
    ...
    private FileConfiguration getYamlConfig(final File file) {
        try {
            ...
            logger.severe("Failed to load YAML file " + file.getName() + ": " + e.getMessage());
    ...
    public void cacheItems() {
        ...
        if (useJson) {
            ...
            scanner.close();
            items.clear();
            final Type typeToken = new TypeToken<HashMap<String, Map<String, Object>>>() {}.getType();
    ...
    public boolean saveDisabledServerRecipes(final List<String> keys) {
        serverRecipeConfig = getYamlConfig(serverRecipeFile);
        if (serverRecipeConfig == null) {
            logger.severe("server-recipes.yml is null! Save failed.");
            return false;
        }
        ...
        try {
            ...
            logger.severe("Failed to save server-recipes.yml: " + e.getMessage());
            return false;
        }
    }
    ...
    public boolean saveContainerOwners(final Map<Location, UUID> blockOwners) {
        containerOwnerConfig = getYamlConfig(containerOwnerFile);
        if (containerOwnerConfig == null) {
            logger.severe("container_owners.yml is null! Save failed.");
            return false;
        }
        for (String key : containerOwnerConfig.getKeys(false)) {
            containerOwnerConfig.set(key, null);
        }
        for (final Map.Entry<Location, UUID> entry : blockOwners.entrySet()) {
            final Location loc = entry.getKey();
            final String key = loc.getBlockX() + "," + loc.getBlockY() + "," + loc.getBlockZ() + "," + loc.getWorld().getUID();
            containerOwnerConfig.set(key, entry.getValue().toString());
        }
        try {
            containerOwnerConfig.save(containerOwnerFile);
            return true;
        } catch (IOException e) {
            logger.severe("Failed to save container_owners.yml: " + e.getMessage());
            return false;
        }
    }
